...
"""
def percent_in(backboard_y,backboard_z):
    ...
    while shots_made < 1:
        random_y =  np.random.uniform(smallest_y,7)
        random_z =  np.random.uniform(1.5,2)
        random_v0 = np.random.uniform(6,10)
        ...
        y_points,z_points,v_y_points,v_z_points = RK4(init,phi_array)
    ...
    print(np.float(num_backboard)/np.float(num_shots))
...
def which_backboard():
    ...
    print(percent_in(backboard_y,backboard_z))
